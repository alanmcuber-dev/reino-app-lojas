    ...
    <div style={{ display: 'flex', minHeight: '100vh', background: '#0a1625' }}>
        ...
        <nav style={{ flex: 1, padding: '8px 0' }}>
          {[
            ...
            <div key={item.label} onClick={() => window.location.href = item.href}
              style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '9px 16px', color: item.href === '/dashboard/integracoes' ? '#c9982a' : '#8ea3be', background: item.href === '/dashboard/integracoes' ? 'rgba(201,152,42,0.1)' : 'transparent', borderLeft: item.href === '/dashboard/integracoes' ? '2px solid #c9982a' : '2px solid transparent', cursor: 'pointer', fontSize: '13px' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
        <div style={{ height: '56px', background: '#0d1b2e', borderBottom: '1px solid rgba(201,152,42,0.1)', display: 'flex', alignItems: 'center', padding: '0 20px' }}>
          <div style={{ color: '#fff', fontSize: '16px', fontWeight: 600 }}>🔌 Integrações</div>
        </div>
        <div style={{ flex: 1, padding: '20px' }}>
          <div style={{ background: '#132236', border: '1px solid rgba(201,152,42,0.1)', borderRadius: '12px', padding: '40px', textAlign: 'center', color: '#8ea3be' }}>
            ...
            <h2 style={{ color: '#fff', marginBottom: '8px' }}>Integrações</h2>
            <p>Em breve você poderá conectar sua loja com outros serviços.</p>
          </div>
        </div>
      </div>
    </div>
